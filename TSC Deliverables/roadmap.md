# COSAI TSC Deliverables Roadmap

> This file tracks all active TSC-level deliverables, their current stage,
> next deadline, and responsible party. It is updated after each TSC meeting
> as deliverables progress through the approval pipeline.
>
> The live view with full timeline is on the
> [GitHub Projects Roadmap Board](../../projects).
> To propose a new deliverable, open an Issue using the
> [Deliverable template](../../issues/new/choose) and label it
> `proposed-deliverable`. Accepted deliverables are promoted to this
> roadmap after TSC discussion.

---

## 📊 Active Deliverables

| # | Deliverable | Workstream / SIG | Current Stage | Next Deadline | Next Milestone |
|---|---|---|---|---|---|
| 1 | Election and Appointment of New TSC Co-Chairs | TSC | 🟢 Published / Complete | 2026-08-28 | Election closed; new co-chairs appointed |
| 2 | Transition of Co-Chair Responsibilities | TSC | 🔵 In Progress | 2026-09-15 & 2026-09-22 | Transition discussions extended to the Sept 15 and Sept 22 meetings |
| 3 | AIMM Paper | WS1 — Software Supply Chain Security for AI Systems | 🟢 Published / Complete | 2026-08-23 | Approved by TSC & PGB full majority vote |
| 4 | Zero Trust Paper | WS2 — Preparing Defenders for a Changing Cybersecurity Landscape | 🟢 Published / Complete | 2026-08-23 | Approved by TSC & PGB full majority vote |
| 5 | Telemetry Paper | WS2 — Preparing Defenders for a Changing Cybersecurity Landscape | 🔵 In Progress | TBD | TSC Co-chairs Review |
| 6 | Agentic Isolation Blog | WS4 — Secure Design Patterns for Agentic Systems | 🟢 Published / Complete | 2026-08-21 | Publication |
| 7 | MCP Security Whitepaper V2 | WS4 — Secure Design Patterns for Agentic Systems | 🟢 Published / Complete | 2026-08-12 | Published; V2.x residuals tracked in [#163](https://github.com/cosai-oasis/ws4-secure-design-agentic-systems/issues/163) |
| 8 | Containment Follow-on Paper | WS4 — Secure Design Patterns for Agentic Systems | 🔵 In Progress | 2026-10-15 | First draft to TSC reviewers; contributions close 2026-10-08 |
| 9 | Agent Credentials Paper | WS4 — Agent Credentials Group | 🔵 In Progress | 2026-10-31 | Early draft skeleton for broader review (October; day not fixed) |
| 10 | Multimodal Threat Taxonomy | WS4 — Multimodal Agentic Security Group | 🔵 In Progress | 2026-10-31 | Refined modality-agnostic taxonomy draft for review. |
| 11 | MCP Risk Map Decomposition Review | WS4 — Secure Design Patterns for Agentic Systems | 🔵 In Progress | TBD | SME review of [secure-ai-tooling#507](https://github.com/cosai-oasis/secure-ai-tooling/pull/507) (76 entries); needs volunteers ([#178](https://github.com/cosai-oasis/ws4-secure-design-agentic-systems/issues/178)) |
| 12 | ADLC Lifecycle Definitional Paper | WS4 — SIG ADLC | 🔵 Planned | TBD | Courtenay Ngo currently owns deliverable |
| 13 | ADLC Risks and Controls | WS4 — SIG ADLC | 🔵 In Progress | Overdue (end of Aug) | Blocked on SIG leadership and the Definitional Paper |

---

## 💡 Proposed Deliverables
> These are proposals submitted by TSC members via GitHub Issues labeled
> `proposed-deliverable`. They are discussed at TSC meetings and promoted
> to Active Deliverables upon acceptance.

| Issue | Proposed Deliverable | Proposer | Workstream / SIG | Status |
|---|---|---|---|---|
| | | | | |

---

## 🔵 Approval Pipeline Stages

Deliverables move through these stages in order:

| Stage | Meaning |
|---|---|
| 🔵 Planned | Agreed at TSC level; work not yet started |
| 🔵 In Progress | Actively being worked on by the responsible workstream or SIG |
| 🟡 TSC Co-chairs Review | Draft submitted; under review by TSC co-chairs |
| 🟠 TSC Review | Under review by full TSC membership |
| 🔴 TSC & PGB Review | Under joint review by TSC and PGB |
| 🟣 TSC Full Majority Vote | Formal TSC full majority vote in progress |
| 🟤 TSC & PGB Full Majority Vote | Formal joint TSC and PGB full majority vote in progress |
| 🗳️ TSC Vote | Active TSC election or appointment vote in progress |
| 🟢 Published / Complete | Finished, published, or appointment confirmed |

---

## 📄 Papers & Points of View

| Title | Workstream / SIG | Owner | Start Date | Target Date | Status | Issue |
|---|---|---|---|---|---|---|
| AIMM Paper | WS1 | TBD | | 2026-08-23 | 🟢 Published / Complete | |
| Zero Trust Paper | WS2 | TBD | | 2026-08-23 | 🟢 Published / Complete | |
| Telemetry Paper | WS2 | TBD | | TBD | 🔵 In Progress | |
| MCP Security Whitepaper V2 | WS4 | Sarah Novotny (@sarahnovotny), Ian Molloy (@imolloy) | | 2026-08-12 | 🟢 Published / Complete | [#141](https://github.com/cosai-oasis/ws4-secure-design-agentic-systems/pull/141) |
| Containment Follow-on Paper | WS4 | Jeff Leva (@Levaj2000), Editor | 2026-08-25 | 2026-10-29 | 🔵 In Progress | [#172](https://github.com/cosai-oasis/ws4-secure-design-agentic-systems/issues/172) |
| Agent Credentials Paper | WS4 — Agent Credentials Group | Benedict Lau (@benhylau), Rithikha Rajamohan (@rithikha) | | 2026-10-31 | 🔵 In Progress | [#99](https://github.com/cosai-oasis/ws4-secure-design-agentic-systems/issues/99) |
| Multimodal Threat Taxonomy | WS4 — Multimodal Agentic Security | Shriti Priya (@monshri) | | TBD (unconfirmed — see Active Deliverables row 10) | 🔵 In Progress | [#113](https://github.com/cosai-oasis/ws4-secure-design-agentic-systems/issues/113) |
| ADLC Lifecycle Definitional Paper | WS4 — SIG ADLC | **Unowned** | | TBD | 🔵 Planned | [#97](https://github.com/cosai-oasis/ws4-secure-design-agentic-systems/issues/97) |

---

## 📝 Blog Posts

| Title | Workstream / SIG | Owner | Start Date | Target Date | Status | Issue |
|---|---|---|---|---|---|---|
| Agentic Isolation Blog | WS4 | Ian Molloy, Sarah Novotny | | 2026-08-21 | 🟢 Published / Complete | |

---

## 🛠 Tools

| Title | Owner | Start Date | Target Date | Status | Issue |
|---|---|---|---|---|---|
| | | | | 🔵 Planned | |

---

## 🎤 Presentations

| Title | Event | Owner | Start Date | Target Date | Status | Issue |
|---|---|---|---|---|---|---|
| | | | | | 🔵 Planned | |

---

## 🚀 Publications & Releases

| Title | Channel | Owner | Start Date | Target Date | Status | Issue |
|---|---|---|---|---|---|---|
| | | | | | 🔵 Planned | |

---

## 🏛️ TSC Governance

| Item | Responsible | Next Deadline | Next Milestone | Status |
|---|---|---|---|---|
| Election and Appointment of New TSC Co-Chairs | TSC | 2026-08-28 | Election closed; new co-chairs appointed | 🟢 Published / Complete |
| Transition of Co-Chair Responsibilities | Akila Srinivasan, J.R. Rao | 2026-09-15 & 2026-09-22 | Transition discussions extended to the Sept 15 and Sept 22 meetings | 🔵 In Progress |

---

## 📌 How to Update This File

1. After each TSC meeting, update the **Current Stage** and **Next Deadline**
   for any deliverable that progressed
2. When a new deliverable is proposed by a TSC member, they open a GitHub
   Issue labeled `proposed-deliverable` — it appears in the Proposed
   Deliverables table above for discussion at the next TSC meeting
3. When a proposed deliverable is accepted by the TSC, move it to the
   Active Deliverables table and update its stage
4. When a deliverable is complete, move it to 🟢 Published / Complete
   and add a link to the published work in the Issue before closing it
5. Commit and push the updated roadmap:
   ```
   git add .
   git commit -m "roadmap: update deliverables status"
   git push
   ```
